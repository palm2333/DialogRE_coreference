...
parser.add_argument('--use_corefchain', type=bool, default=False)
...
def GetVertexSet_cc(reg_infos,entity_data, doc):
    length_per_sent = [len(cut(sent)) for sent in doc]
    entities = []
    for entity in ['x', 'y']:
        xcoref = []
        e_info = entity_data[entity + "_mention"]
        if e_info!=[]:
            turns = e_info['turn']
            mentions = e_info['mention']
            for i in range(len(e_info['turn'])):
                acoref = {}
                acoref['name'] = entity_data[entity]
                acoref['mention'] = mentions[i]
                acoref['sent_id'] = turns[i] - 1
                acoref['type'] = entity_data[entity + "_type"]


                if i == 0 or (turns[i - 1] != turns[i]):
                    wheres = {}
                    last_idx = {}
                    visited = [0] * length_per_sent[turns[i] - 1]
                if turns[i] <= len(doc):
                    sent = doc[turns[i] - 1]
                    mention = mentions[i]
                    where = find_all_indexes(sent, mention)
                if mention not in wheres.keys():
                    wheres[mention] = where
                    last_idx[mention] = -1
                for k in range(last_idx[mention] + 1, len(where)):
                    pos = where[k]
                    if ((pos == 0 and is_punc_or_blank(sent[pos + len(mention)])) or \
                            (pos + len(mention) > len(sent) - 1 and is_punc_or_blank(sent[pos - 1])) or \
                            (is_punc_or_blank(sent[pos - 1]) and is_punc_or_blank(sent[pos + len(mention)]))):
                        left_idx = len(cut(sent[:pos]))
                        if is_punc_or_blank(sent[pos - 1]) and sent[pos - 1] != " ":
                            left_idx-=2
                        # 
                        length = len(cut(sent[pos:pos + len(mention)]))
                        # 
                        if not_visit(visited, left_idx, length) == True:
                            last_idx[mention] = k
                            visited[left_idx:left_idx + length] = [1] * length
                            acoref['pos'] = [left_idx, left_idx + length]
                            xcoref.append(acoref)
                            break
        # 
        reg_info = reg_infos[0] if entity=='x' else reg_infos[1]
        for reg in reg_info:
            if reg not in xcoref:
                xcoref.append(reg)
        entities.append(xcoref)
    return entities
...
def Init(data_file_name, max_length=512, is_training=True, suffix=''):
    ...
    for i in range(len(ori_data)):
        ...
        for instance in ori_data[i][1]:
            ...
            entities = [(instance['x'], instance['x_type']), (instance['y'], instance['y_type'])]
            entities = GetVertexSet(doc, entities)
            if args.use_corefchain:
                entities = GetVertexSet_cc(entities, instance, ori_data[i][0])
    ...
    # ori_data=[[[sentences],[{x,y,rid,...}]],[other dialog]]
...
Init(train_file_name, max_length=700, is_training=False, suffix='_train')
# dev_dev_xxx.npy
print("=========================start to generate the dev instances=========================")
Init(dev_file_name, max_length=700, is_training=False, suffix='_dev')
# # dev_test_xxx.npy
print("=========================start to generate the test instances=========================")
Init(test_file_name,max_length=700, is_training=False, suffix='_test')
