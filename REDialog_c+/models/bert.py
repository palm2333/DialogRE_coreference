...
class Bert():
    ...
    def __init__(self, model_class, model_name):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(path+"/"+"vocab.txt")
        modelConfig = BertConfig.from_pretrained(path+"/"+"bert_config.json")
        self.model = model_class.from_pretrained(
            path+"/"+'pytorch_model.bin', config=modelConfig).cuda()
        ...
        if sen_seg:
            ...
            for sent in tokens:
                ...
                if len(subwords) < 510:
                    ...
                    token_start_idxs[1] = np.concatenate((token_start_idxs[1], len(subwords) + 1 + np.cumsum([0] + subword_lengths[:-1])), axis=0)
