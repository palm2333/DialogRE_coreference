...
def f1c(devp, data):
    ...
    for i in range(len(data)):
        for j in range(len(data[i][1])):
            ...
            # l: 对于f1c一个三元组,除unans外gold_rid,()
            l = set(data[i][1][j]["rid"]) - set([36])
            # et: 对于f1c一个三元组，rid外=true, rid: false
            ex, ey = False, False
            et = {}
            for r in range(36):
                et[r] = r not in l
            # o: 对于一个f1c三元组, 除unans外pred_rid,()
            for k in range(len(data[i][0])):
                o = set(devp[index]) - set([36])
                e = set()                       # 对话完整e:36个rid, 否则e: x|y未出现=空，x&y出现=36-gold_rid无trigger-gold_rid的trigger未出现
                if x in data[i][0][k].lower():   # ex: x在此三元组对话中是否出现
                    ex = True
                if y in data[i][0][k].lower():    # ey: y在此三元组对话中是否出现
                    ey = True
                if k == len(data[i][0])-1:        # 三元组为完整对话
                    ex = ey = True
                    for r in range(36):
                        et[r] = True
                for r in range(36):
                    if r in t:                    # rid有trigger，且在此三元组对话中出现
