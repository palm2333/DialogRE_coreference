...
class Dataset(data.Dataset):
    ...
    def creat_graph(self, utters, utters_ner, x, y, pos_tags,x_mention,y_mention):
        ...
        graph: dgl.DGLGraph
            ...
            edge:
                ...
                sent & word &speaker- coref: dtype=2
        """
        G = dgl.graph([])
        ...
        # speaker-sentence
        speaker_ids = [int(o[1])-1 if o[1].isdigit() else int(o[1][0])-1 for o in utters] # speaker id start from 0
        ...
        # argument - ner edge: argument - ner
        for wid in arga_wordid:
        ...
        if x_mention!=[]:
            for co in x_mention:
                if isinstance(co, str):
                    ...
                    print(co)
                sent_id=co['sent_id']
                if sent_id < len(sid2nid):
                    G.add_edges(argid2nid[0], sid2nid[sent_id],data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                    G.add_edges(sid2nid[sent_id], argid2nid[0],data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                else:
                    break
        if y_mention!=[]:
            for co in y_mention:
                sent_id = co['sent_id']
                if sent_id < len(sid2nid):
                    G.add_edges(argid2nid[1], sid2nid[sent_id],data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                    G.add_edges(sid2nid[sent_id], argid2nid[1],data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                else:
                    break
        ...
        if x_mention != []:
            for co in x_mention:
                sent_id = co['sent_id']
                pos=co['pos']
                ids = utters_ids[sent_id][pos[0]:pos[1]]
                # print(utters[sent_id][pos[0]:pos[1]])
                for id in ids:
                    G.add_edges(wid2nid[int(id)], argid2nid[0], data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                    G.add_edges(argid2nid[0], wid2nid[int(id)], data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
        if y_mention != []:
            for co in y_mention:
                sent_id = co['sent_id']
                pos=co['pos']
                ...
                # print(utters[sent_id][pos[0]:pos[1]])
                for id in ids:
                    G.add_edges(wid2nid[int(id)], argid2nid[1], data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                    G.add_edges(argid2nid[1], wid2nid[int(id)], data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})

        # speaker-coref
        if x.startswith("Speaker"):
            s_id=int(x[-1])
            if s_id<=len(speakerid2nid):
                G.add_edges(argid2nid[0], speakerid2nid[s_id-1],
                            data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                G.add_edges(speakerid2nid[s_id-1], argid2nid[0],
                            data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
        if y.startswith("Speaker"):
            s_id=int(y[-1])
            if s_id<=len(speakerid2nid):
                G.add_edges(argid2nid[1], speakerid2nid[s_id-1],
                            data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
                G.add_edges(speakerid2nid[s_id-1], argid2nid[1],
                            data={'ws_link': self.get_weight(), "dtype": torch.tensor([2])})
    ...
    def __getitem__(self, index):
        ...
        # if 'x_coref' not in self.data[index]:
        #     print("hh")
        #     print(self.data[index]['x'])
        #     print(self.data[index]['y'])
        item['x_node_id'], item['y_node_id'], item['conv_graph'] = self.creat_graph(
            ...
            self.data[index]['x_coref'],
            self.data[index]['y_coref'],
        ) 
        return item
