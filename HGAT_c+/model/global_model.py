...
class ConvEncoder(nn.Module):
    ...
    def __init__(self, config, embeddings):
        ...
        # 
        self.co_embed= nn.Embedding(config.co_edge_bucket, config.edge_embed_size) # bucket = 10
    ...
    def forward(self, graph, batch, local_feature):
        ...
        # 
        self.set_corefEdge_feature(graph)
    ...
    # utter-coref, word-coref
    def set_corefEdge_feature(self,graph):
        coedge_id=graph.filter_edges(lambda edges: edges.data["dtype"] == 2)
        co_edge = graph.edges[coedge_id].data['ws_link']
        graph.edges[coedge_id].data['wn_embed'] = self.co_embed(co_edge)