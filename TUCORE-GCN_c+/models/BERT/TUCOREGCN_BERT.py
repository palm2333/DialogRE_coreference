...
class TUCOREGCN_BERT(nn.Module):
    def __init__(self, config, num_labels, gcn_layers=3, activation='relu', gcn_dropout=0.6):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size*5*(self.gcn_layers+1), self.num_labels)
        ...
        rel_name_lists = ['speaker', 'dialog', 'entity', 'coref', 'chain']
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, speaker_ids, graphs, mention_id, labels=None, turn_mask=None, coref_ids=None):
        slen = input_ids.size(1)
        sequence_outputs, pooled_outputs = self.bert(input_ids, speaker_ids, token_type_ids, attention_mask)

        a=torch.zeros_like(coref_ids)
        b = torch.zeros_like(coref_ids)
        coref_masks=(coref_ids>0).float()
        coref_outputs,_=self.bert(coref_ids,a,b,coref_masks)
        ...
        for i in range(len(graphs)): 
            ...
            where_101 = get_index_list(coref_ids[i], 101)
            where_h = [where_101[0]] + get_index_list(coref_ids[i][:where_101[1]], 102)
            where_t = [where_101[1]] + [k + where_101[1] for k in get_index_list(coref_ids[i][where_101[1]:], 102)]
            for where in [where_h, where_t]:
                coref_node = None
                for j in range(len(where) - 1):
                    if coref_node is None:
                        coref_node = coref_outputs[i][where[j] + 1:where[j + 1]].mean(axis=0)
                        coref_node = coref_node.unsqueeze(0)
                    else:
                        coref_node = torch.cat((coref_node, (coref_outputs[i][where[j] + 1:where[j + 1]].mean(axis=0)).unsqueeze(0)),dim=0)
                if where==where_h:
                    coref_nodes['h'].append(coref_node)
                else:
                    coref_nodes['t'].append(coref_node)
                if coref_node is not None:
                    features = torch.cat((features,coref_node), dim=0)
        ...
        for layer_num, GCN_layer in enumerate(self.GCN_layers):
            start = 0
            new_features = []
            for i in range(len(num_batch_turn)):
                idx=num_batch_turn[i]
                ...
                start += idx
                if coref_nodes['h'][i] is not None:
                    new_features+=features[start:start+len(coref_nodes['h'][i])]
                    start += len(coref_nodes['h'][i])
                if coref_nodes['t'][i] is not None:
                    new_features+=features[start:start+len(coref_nodes['t'][i])]
                    start += len(coref_nodes['t'][i])
        ...
        fea_idx = 0
        for i in range(len(graphs)):
            num_exc_coref = num_batch_turn[i]
            ...
            for j in range(self.gcn_layers + 1):
                ...
                intergrated_output = torch.cat((intergrated_output, output_features[j][fea_idx + num_exc_coref - 2]), dim=-1)
                intergrated_output = torch.cat((intergrated_output, output_features[j][fea_idx + num_exc_coref - 1]), dim=-1)
                if coref_nodes['h'][i] is not None:
                    coref_node = output_features[j][fea_idx + num_exc_coref:fea_idx + num_exc_coref+len(coref_nodes['h'][i])].mean(axis=0)
                    intergrated_output = torch.cat((intergrated_output, coref_node), dim=-1)
                else:
                    intergrated_output = torch.cat((intergrated_output, get_cuda(torch.tensor([0]*768))), dim=-1)
                if coref_nodes['t'][i] is not None:
                    if coref_nodes['h'][i] is not None:
                        coref_node = output_features[j][fea_idx + num_exc_coref+len(coref_nodes['h'][i]):fea_idx + num_exc_coref+len(coref_nodes['h'][i])+len(coref_nodes['t'][i])].mean(axis=0)
                    else:
                        coref_node = output_features[j][fea_idx + num_exc_coref :fea_idx + num_exc_coref + len(coref_nodes['t'][i])].mean(axis=0)
                    intergrated_output = torch.cat((intergrated_output, coref_node), dim=-1)
                else:
                    intergrated_output = torch.cat((intergrated_output, get_cuda(torch.tensor([0] * 768))), dim=-1)
            fea_idx += graphs[i].number_of_nodes('node')
            graph_output.append(intergrated_output)
        graph_output=pad_sequence(graph_output, batch_first=True, padding_value=0)
