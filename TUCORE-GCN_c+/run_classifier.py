...
def calc_test_result(logits, labels_all):
    true_label=[]
    predicted_label=[]
...
def get_logits4eval(model, dataloader, savefile, device):
    ...
    for batch in tqdm(dataloader, desc="Iteration"):
        ...
        graphs = batch['graphs']
        coref_ids = batch['coref_ids'].to(device)

        with torch.no_grad():
            tmp_eval_loss, logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask, coref_ids=coref_ids)
...
def main():
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                loss, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask, coref_ids=coref_ids)
                if n_gpu > 1:
                    loss = loss.mean()
    ...
    if args.do_eval:
        # for f1
        dev_set = TUCOREGCNDataset(src_file=args.data_dir,
                                   save_file=args.data_dir + "/dev_" + args.encoder_type + ".pkl",
                                   max_seq_length=args.max_seq_length, tokenizer=tokenizer, n_class=n_class,
                                   encoder_type=args.encoder_type)
        dev_loader = TUCOREGCNDataloader(dataset=dev_set, batch_size=args.eval_batch_size, shuffle=False,
                                         relation_num=n_class, max_length=args.max_seq_length)
        test_set = TUCOREGCNDataset(src_file=args.data_dir,
                                    save_file=args.data_dir + "/test_" + args.encoder_type + ".pkl",
                                    max_seq_length=args.max_seq_length, tokenizer=tokenizer, n_class=n_class,
                                    encoder_type=args.encoder_type)
        test_loader = TUCOREGCNDataloader(dataset=test_set, batch_size=args.eval_batch_size, shuffle=False,
                                          relation_num=n_class, max_length=args.max_seq_length)

        # for f1c
        devc_set = TUCOREGCNDataset4f1c(src_file=args.data_dir,
                                        save_file=args.data_dir + "/devc_" + args.encoder_type + ".pkl",
                                        max_seq_length=args.max_seq_length, tokenizer=tokenizer, n_class=n_class,
                                        encoder_type=args.encoder_type)
        devc_loader = TUCOREGCNDataloader(dataset=devc_set, batch_size=args.eval_batch_size, shuffle=False,
                                          relation_num=n_class, max_length=args.max_seq_length)
        testc_set = TUCOREGCNDataset4f1c(src_file=args.data_dir,
                                         save_file=args.data_dir + "/testc_" + args.encoder_type + ".pkl",
                                         max_seq_length=args.max_seq_length, tokenizer=tokenizer, n_class=n_class,
                                         encoder_type=args.encoder_type)
        testc_loader = TUCOREGCNDataloader(dataset=testc_set, batch_size=args.eval_batch_size, shuffle=False,
                                           relation_num=n_class, max_length=args.max_seq_length)
