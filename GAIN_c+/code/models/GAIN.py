...
class GAIN_GloVe(nn.Module):
    ...
    def forward(self, **params):
        '''
            ...
            entity_type_all: [batch_size, max_length]
            entity_id_all: [batch_size, max_length]
            mention_id_all: [batch_size, max_length]
        ...
        if self.config.use_entity_type:
            # 修改
            ner_id_feature = torch.max(self.entity_type_emb(params['entity_type_all']), 1)[0]
            src = torch.cat([src, ner_id_feature], dim=-1)

        if self.config.use_entity_id:
            # 修改
            pos_id_feature = torch.max(self.entity_id_emb(params['entity_id_all']), 1)[0]
            src = torch.cat([src, pos_id_feature], dim=-1)
        ...
        mention_id = params['mention_id_all']
        features = None
        ...
        for i in range(len(graphs)):
            ...
            # 修改
            mention_id_i = mention_id[i]
            mentions = torch.zeros(mention_index.size())
            pre = 0
            for j in range(len(mention_id_i)):
                mention = mention_id_i[j]
                num = mention.max() - pre
                if num < 0: num = 0
                mentions[pre:pre + num] = mention_id_i[j].unsqueeze(0).expand(num, -1)
                pre = pre + num
            mentions = get_cuda(mentions)
        ...
        entity_num = torch.max(params['entity_id_all'])
        ...
        for i in range(len(entity_graphs)):
            ...
            for j in range(h_t_limit):
                ...
                if v == []:
                    # forward
                    edge_ids = random.randint(0, len(entity_graphs[i].edges()[0]) - 1)
...
class GAIN_BERT(nn.Module):
    ...
    def forward(self, **params):
        ...
        src_lengths: [batchs_size]
        mask: [batch_size, max_length]
        entity_type_all: [batch_size, entity_num, max_length]
        entity_id_all: [batch_size, entity_num,max_length]
        mention_id_all: [batch_size, entity_num, max_length]
        distance: [batch_size, max_length]
        entity2mention_table: list of [local_entity_num, local_mention_num]
        graphs: list of DGLHeteroGraph
        h_t_pairs: [batch_size, h_t_limit, 2]
        ...
        if self.config.use_entity_type:
            ner_id_feature=torch.max(self.entity_type_emb(params['entity_type_all']), 1)[0]
            encoder_outputs = torch.cat([encoder_outputs, ner_id_feature], dim=-1)

        if self.config.use_entity_id:
            pos_id_feature=torch.max(self.entity_id_emb(params['entity_id_all']), 1)[0]
            encoder_outputs = torch.cat([encoder_outputs, pos_id_feature], dim=-1)
        ...
        for i in range(len(graphs)):
            # average mention -> entity
            # entity_num+1 * mention_num+1, 每行对应一个entity，对应mention位置=1
            select_metrix = entity2mention_table[i].float()  # [local_entity_num, mention_num]
            select_metrix[0][0] = 1
            mention_nums = torch.sum(select_metrix, dim=-1).unsqueeze(-1).expand(-1, select_metrix.size(1))
            select_metrix = torch.where(mention_nums > 0, select_metrix / mention_nums, select_metrix)
...
class BiLSTM(nn.Module):
    ...
    def forward(self, src, src_lengths):
        ...
        # 修改
        packed_src = nn.utils.rnn.pack_padded_sequence(new_src, new_src_lengths.to('cpu'), batch_first=True, enforce_sorted=True)
